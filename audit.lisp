...
(defun switchable? (takenclasses required-courses)
  (let ((or-classes (member-if #'listp required-courses)))
    (if (null or-classes)
      (check-required-classes (cdr takenclasses) required-courses)
      (dolist (or-now or-classes (check-required-classes (cdr takenclasses) required-courses))
        (if (member (car takenclasses) or-now)
          (return (check-required-classes (cdr takenclasses) (remove or-now required-courses)))
        )
      )
    )
  )
...
(defun check-required-classes (takenclasses required-courses)
  (if (null takenclasses)
    required-courses
    (if (member (car takenclasses) required-courses)
      (check-required-classes (cdr takenclasses) (remove (car takenclasses) required-courses))
      (switchable? takenclasses required-courses)
    )
  )
)
